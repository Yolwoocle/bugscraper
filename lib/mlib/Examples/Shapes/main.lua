...
local function generateRandomPolygon(numberOfPoints, minimumX, minimumY, maxWidth, maxHeight)
    ...
    for i = 1, numberOfPoints do
        ...
        while isInTable(x, X) do
            X = love.math.random(minimumX, maximumX)
        end
        while isInTable(y, Y) do
            Y = love.math.random(minimumX, maximumX)
        end
        x[i] = X
        y[i] = Y
    end
    ...
    local function orderPoints(x, y)
        ...
        local smallestX, smallestReference = sortWithReference(x, function(v1, v2)
            return v1 > v2
        end)
        local smallestXY = y[smallestReference]
        table.remove(x, smallestReference)
        table.remove(y, smallestReference)
        local largestX, largestReference = sortWithReference(x, function(v1, v2)
            return v1 < v2
        end)
        ...
        while #x > 0 do
            ...
            if crossProduct >= 0 then
                upperX[#upperX + 1], upperY[#upperY + 1] = X, Y
            else
                lowerX[#lowerX + 1], lowerY[#lowerY + 1] = X, Y
            end
        ...
        while #upperX > 0 do
            local x, Reference = sortWithReference(upperX, function(v1, v2)
                return v1 > v2
            end)
        ...
        while #lowerX > 0 do
            local x, Reference = sortWithReference(lowerX, function(v1, v2)
                return v1 < v2
            end)
            newOrder[#newOrder + 1] = x
...
local function lgprint(x, y, ...)
    ...
    table.foreach(text, function(_, str)
        str = tostring(str)
    ...
    shapes = {
        {
            ...
            points = generateRandomPolygon(
                ...
                love.math.random(0, width / 4),
                love.math.random(0, height / 4),
                love.math.random(100, width / 4),
                love.math.random(100, height / 4)
            ),
        },
        {
            type = "polygon",
            ...
            points = generateRandomPolygon(
                love.math.random(3, 15),
                love.math.random(width / 4, width / 2),
                love.math.random(height / 4, height / 2),
                love.math.random(100, width / 4),
                love.math.random(100, height / 4)
            ),
        },
        {
            ...
            points = generateRandomPolygon(
                love.math.random(3, 15),
                love.math.random(width / 2, 3 * width / 4),
                love.math.random(height / 2, 3 * height / 4),
                love.math.random(100, width / 4),
                love.math.random(100, height / 4)
            ),
        },
        {
            ...
            points = generateRandomPolygon(
                love.math.random(3, 15),
                love.math.random(width / 4, width),
                love.math.random(height / 4, height),
                love.math.random(100, width / 4),
                love.math.random(100, height / 4)
            ),
        },
...
function love.update(dt)
    ...
    for i, shape in ipairs(shapes) do
        ...
        elseif shape.type == "polygon" then
            ...
            if mlib.polygon.checkPoint(mx, my, shape.points) then
                shape.selected = true
            else
                shape.selected = false
            end
            ...
            if i ~= ii then
                if shape.type == "polygon" then
                    ...
                    elseif shape2.type == "circle" then
                        local intersections =
                            mlib.polygon.getCircleIntersection(shape2.x, shape2.y, shape2.radius, shape.points)
                        if intersections then
                            intersection = true
                            shape.collided = true
                            shape2.collided = true
                    ...
                    elseif shape2.type == "line" then
                        local intersections = mlib.polygon.getSegmentIntersection(
                            shape2.x1,
                            shape2.y1,
                            shape2.x2,
                            shape2.y2,
                ...
                elseif shape.type == "circle" then
                    if shape2.type == "polygon" then
                        local intersections =
                            mlib.polygon.getCircleIntersection(shape.x, shape.y, shape.radius, shape2.points)
                        if intersections then
                            intersection = true
                            shape.collided = true
                            shape2.collided = true
                    ...
                    elseif shape2.type == "circle" then
                        local _, x1, y1, x2, y2 = mlib.circle.getCircleIntersection(
                            shape.x,
                            shape.y,
                            shape.radius,
                            shape2.x,
                    ...
                    elseif shape2.type == "line" then
                        local _, x1, y1, x2, y2 = mlib.circle.getSegmentIntersection(
                            shape.x,
                            shape.y,
                            shape.radius,
                            shape2.x1,
                ...
                elseif shape.type == "line" then
                    if shape2.type == "polygon" then
                        local intersections =
                            mlib.polygon.getSegmentIntersection(shape.x1, shape.y1, shape.x2, shape.y2, shape2.points)
                        if intersections then
                            intersection = true
                            shape2.collided = true
                            shape.collided = true
                    ...
                    elseif shape2.type == "line" then
                        local x1, y1, x2, y2 = mlib.segment.getIntersection(
                            shape.x1,
                            shape.y1,
                            shape.x2,
                            shape.y2,
...
function love.draw()
    ...
    for _, shape in ipairs(shapes) do
        ...
        love.graphics.setColor(255, 255, 255, 255)
        if DEBUG then
            x, y = printInformation(shape, x, y)
        end
    end
...
function love.keypressed(key)
    ...
    elseif
        key ~= "printscreen"
        and key ~= "lctrl" -- Take pictures for debugging.
        and key ~= "lalt"
        and key ~= "tab"
    then -- Switch between views without changing view.
        love.load()
    end
end
