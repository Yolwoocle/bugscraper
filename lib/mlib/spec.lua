...
context("mlib", function()
    ...
    local function check_fuzzy(a, b)
        return (a - 0.00001 <= b and b <= a + 0.00001)
    end
    ...
    local function DeepCompare(Table1, Table2)
        ...
        for Key, Value in pairs(Table1) do
            if type(Value) == "table" and type(Table2[Key]) == "table" then
                if not DeepCompare(Value, Table2[Key]) then
                    return false
                end
            else
        ...
        for Key, Value in pairs(Table2) do
            if type(Value) == "table" and type(Table1[Key]) == "table" then
                if not DeepCompare(Value, Table1[Key]) then
                    return false
                end
            else
                if type(Value) ~= type(Table1[Key]) then
                    return false
                end
                if type(Value) == "number" then
                    return check_fuzzy(Value, Table1[Key])
                elseif Value ~= Table1[Key] then
                    return false
                end
            end
        end
    ...
    make_assertion("tables_fuzzy_equal", "all table values are equal", function(Table1, Table2)
    ...
    context("line", function()
        context("getLength", function()
            test("Gives the length of a line.", function()
                ...
                assert_fuzzy_equal(_.line.getLength(9.3, 7.6, -12, 0.001), 22.61492)
        ...
        context("getDistance", function()
            test("Alias for getLength.", function()
                ...
                assert_fuzzy_equal(_.line.getDistance(9.3, 7.6, -12, 0.001), _.line.getLength(9.3, 7.6, -12, 0.001))
                assert_fuzzy_equal(
                    _.line.getDistance(4.2, 4.134, 7.2342, -78),
                    _.line.getLength(4.2, 4.134, 7.2342, -78)
                )
            end)
        end)
        ...
        context("getMidpoint", function()
            test("Gives the midpoint of a line.", function()
                ...
                assert_multiple_fuzzy_equal(
                    { _.line.getMidpoint(3.14159, 3.14159, 2.71828, 2.71828) },
                    { 2.92993, 2.92993 }
                )
            end)
        end)
        ...
        context("getPerpendicularSlope", function()
            ...
            test("Gives the perpendicular slope given the slope.", function()
                assert_fuzzy_equal(_.line.getPerpendicularSlope(2), -0.5)
            end)
        ...
        context("getIntersection", function()
            test("Given the slope, y-intercept, and two points of other line.", function()
                assert_multiple_fuzzy_equal({ _.line.getIntersection(1, 0, 1, 0, 0, 1) }, { 0.5, 0.5 })
            end)

            test("Given the slope, y-intercept, the other slope and y-intercept.", function()
                assert_multiple_fuzzy_equal({ _.line.getIntersection(1, 0, -1, 1) }, { 0.5, 0.5 })
            end)

            test("Given two points on one line and two on the other.", function()
                assert_multiple_fuzzy_equal({ _.line.getIntersection(1, 1, 0, 0, 1, 0, 0, 1) }, { 0.5, 0.5 })
            end)
        ...
        context("getClosestPoint", function()
            ...
            test("Given the the point and the slope and y-intercept.", function()
                ...
                assert_multiple_fuzzy_equal({ _.line.getClosestPoint(-1, 3, 0.5, 1) }, { 0, 1 })
            end)
        end)
        ...
        context("getCircleIntersection", function()
            test("Returns 'Secant' when intersects twice.", function()
                assert_multiple_fuzzy_equal(
                    { _.line.getCircleIntersection(4, 9, 1, 0, 9, 6, 9) },
                    { "secant", 3, 9, 5, 9 }
                )
                assert_multiple_fuzzy_equal(
                    { _.line.getCircleIntersection(2, 2, 1, 2, 3, 3, 2) },
                    { "secant", 2, 3, 3, 2 }
                )
            end)
        ...
        context("getLineIntersection", function()
            test("Given the slope, y-intercept, and two points of other line.", function()
                assert_multiple_fuzzy_equal({ _.line.getLineIntersection(1, 0, 1, 0, 0, 1) }, { 0.5, 0.5 })
            end)

            test("Given the slope, y-intercept, the other slope and y-intercept.", function()
                assert_multiple_fuzzy_equal({ _.line.getLineIntersection(1, 0, -1, 1) }, { 0.5, 0.5 })
                assert_multiple_fuzzy_equal({ _.line.getLineIntersection(2, -11, -1, 19) }, { 10, 9 })
            end)

            test("Given two points on one line and two on the other.", function()
                assert_multiple_fuzzy_equal({ _.line.getLineIntersection(1, 1, 0, 0, 1, 0, 0, 1) }, { 0.5, 0.5 })
            end)
        ...
        context("segment", function()
            context("checkPoint", function()
                test("Returns true if the point is on the segment.", function()
                    ...
                    assert_true(_.segment.checkPoint(-0.5, 2, -1, 4, 0, 0))
                end)
            ...
            context("getPerpendicularBisector", function()
                test("Returns the midpoint and perpendicular slope given two points.", function()
                    ...
                    assert_multiple_fuzzy_equal({ _.segment.getPerpendicularBisector(4, 4, 6, 8) }, { 5, 6, -0.5 })
                end)
            ...
            context("getIntersection", function()
                test("Returns the point of intersection if they do.", function()
                    assert_multiple_fuzzy_equal(
                        { _.segment.getIntersection(1, 1, 5, 3, 2, 3, 4, 1) },
                        { 3, 2, nil, nil }
                    )
                    assert_multiple_fuzzy_equal(
                        { _.segment.getIntersection(0, 0, 3, 3, 0, 1, 3, 1) },
                        { 1, 1, nil, nil }
                    )
                end)
                ...
                test("Returns false if they don't.", function()
                    assert_multiple_fuzzy_equal(
                        { _.segment.getIntersection(3, 7, 6, 8, 1, 6, 5, 4) },
                        { false, nil, nil, nil }
                    )
                end)
            ...
            context("getCircleIntersection", function()
                test("Returns 'Secant' if the line connects two points.", function()
                    assert_multiple_fuzzy_equal(
                        { _.segment.getCircleIntersection(4, 9, 1, 0, 9, 6, 9) },
                        { "secant", 3, 9, 5, 9 }
                    )
                end)
                ...
                test("Returns 'Tangent' if the line attaches only one point.", function()
                    assert_multiple_fuzzy_equal(
                        { _.segment.getCircleIntersection(1, 1, 1, 0, 0, 0, 2) },
                        { "tangent", 0, 1 }
                    )
                end)
                ...
                test("Returns 'Chord' if both points are on the circle.", function()
                    assert_multiple_fuzzy_equal(
                        { _.segment.getCircleIntersection(0, 0, 1, -1, 0, 1, 0) },
                        { "chord", -1, 0, 1, 0 }
                    )
                end)
                ...
                test("Returns 'Enclosed' if the line is within the circle entirely.", function()
                    assert_multiple_fuzzy_equal(
                        { _.segment.getCircleIntersection(0, 0, 2, -1, 0, 1, 0) },
                        { "enclosed", -1, 0, 1, 0 }
                    )
                end)
            ...
            context("getPolygonIntersection", function()
                test("Returns the points of intersection.", function()
                    assert_tables_fuzzy_equal(
                        _.segment.getPolygonIntersection(4, 2, 6, 4, 3, 4, 4, 6, 8, 4, 7, 2),
                        { { 5, 3 } }
                    )
                    assert_tables_fuzzy_equal(
                        _.segment.getPolygonIntersection(4, 2, 8, 6, 3, 4, 4, 6, 8, 4, 7, 2),
                        { { 6.66666, 4.66666 }, { 5, 3 } }
                    )
                ...
                test("Works with collinear lines.", function()
                    assert_tables_fuzzy_equal(
                        _.segment.getPolygonIntersection(2, 7, 10, 3, 3, 4, 4, 6, 8, 4, 7, 2),
                        { { 4, 6 }, { 8, 4 } }
                    )
                ...
                test("Works with vertical lines (on poly. and/or segment).", function()
                    assert_tables_fuzzy_equal(
                        _.segment.getPolygonIntersection(6, 2, 6, 6, 3, 4, 4, 6, 8, 4, 7, 2),
                        { { 6, 2.5 }, { 6, 5 } }
                    )
                    assert_tables_fuzzy_equal(
                        _.segment.getPolygonIntersection(6, 4, 10, 4, 3, 4, 4, 6, 8, 5, 8, 3),
                        { { 8, 4 } }
                    )
                    assert_tables_fuzzy_equal(
                        _.segment.getPolygonIntersection(8, 1, 8, 4, 3, 4, 4, 6, 8, 5, 8, 3),
                        { { 8, 3, 8, 4 } }
                    )
            ...
            context("getLineIntersection", function()
                ...
                test("Works with collinear lines.", function()
                    assert_multiple_fuzzy_equal(
                        { _.segment.getLineIntersection(0, 0, 2, 2, -1, -1, 3, 3) },
                        { 0, 0, 2, 2 }
                    )
                end)
            end)
            ...
            context("getSegmentIntersection", function()
                test("Returns the point of intersection if they do.", function()
                    assert_multiple_fuzzy_equal(
                        { _.segment.getSegmentIntersection(1, 1, 5, 3, 2, 3, 4, 1) },
                        { 3, 2, nil, nil }
                    )
                    assert_multiple_fuzzy_equal(
                        { _.segment.getSegmentIntersection(0, 0, 3, 3, 0, 1, 3, 1) },
                        { 1, 1, nil, nil }
                    )
                end)
                ...
                test("Returns false if they don't.", function()
                    assert_multiple_fuzzy_equal(
                        { _.segment.getSegmentIntersection(3, 7, 6, 8, 1, 6, 5, 4) },
                        { false, nil, nil, nil }
                    )
                end)
                ...
                test("Return x1, y1, x2, y2 if lines have same slope and intercept.", function()
                    assert_multiple_fuzzy_equal(
                        { _.segment.getSegmentIntersection(0, 0, 2, 2, 1, 1, 3, 3) },
                        { 2, 2, 1, 1 }
                    )
                    assert_multiple_fuzzy_equal(
                        { _.segment.getSegmentIntersection(0, 1, 4, 1, 2, 1, 3, 1) },
                        { 2, 1, 3, 1 }
                    )
                end)
            end)
            ...
            context("isSegmentCompletelyInsidePolygon", function()
                test("Returns if a segment is completely inside of a polygon.", function()
                    assert_true(
                        _.segment.isSegmentCompletelyInsidePolygon(
                            0,
                            0.5,
                            0.5,
                            0,
    ...
    context("polygon", function()
        ...
        context("getCentroid", function()
            test("Gives the x and y of the centroid.", function()
                ...
                assert_multiple_fuzzy_equal(
                    { _.polygon.getCentroid(2, -1, 2, 1, 1, 2, -1, 2, -2, 1, -2, -1, -1, -2, 1, -2) },
                    { 0, 0 }
                )
        ...
        context("getSegmentIntersection", function()
            test("Returns the points of intersection.", function()
                assert_tables_fuzzy_equal(
                    _.polygon.getSegmentIntersection(4, 2, 6, 4, 3, 4, 4, 6, 8, 4, 7, 2),
                    { { 5, 3 } }
                )
                assert_tables_fuzzy_equal(
                    _.polygon.getSegmentIntersection(4, 2, 8, 6, 3, 4, 4, 6, 8, 4, 7, 2),
                    { { 6.66666, 4.66666 }, { 5, 3 } }
                )
            ...
            test("Works with collinear lines.", function()
                assert_tables_fuzzy_equal(
                    _.polygon.getSegmentIntersection(2, 7, 10, 3, 3, 4, 4, 6, 8, 4, 7, 2),
                    { { 4, 6 }, { 8, 4 } }
                )
            ...
            test("Works with vertical lines (on poly. and/or segment).", function()
                assert_tables_fuzzy_equal(
                    _.polygon.getSegmentIntersection(6, 2, 6, 6, 3, 4, 4, 6, 8, 4, 7, 2),
                    { { 6, 2.5 }, { 6, 5 } }
                )
                assert_tables_fuzzy_equal(
                    _.polygon.getSegmentIntersection(6, 4, 10, 4, 3, 4, 4, 6, 8, 5, 8, 3),
                    { { 8, 4 } }
                )
                assert_tables_fuzzy_equal(
                    _.polygon.getSegmentIntersection(8, 1, 8, 4, 3, 4, 4, 6, 8, 5, 8, 3),
                    { { 8, 3, 8, 4 } }
                )
        ...
        context("getPolygonIntersection", function()
            test("Returns true if the polygons intersect.", function()
                ...
                tab = _.polygon.getPolygonIntersection(
                    { 3, 5, 4, 4, 3, 3, 2, 3, 1, 4, 1, 2, 3, 2, 5, 4, 3, 6, 1, 6 },
                    { 0, 6, 4, 5, 2, 4 }
                )
            ...
            test("Returns false if the polygons don't intersect.", function()
                ...
                assert_false(
                    _.polygon.getPolygonIntersection(
                        { 3, 5, 4, 4, 3, 3, 2, 3, 1, 4, 1, 2, 3, 2, 5, 4, 3, 6, 1, 6 },
                        { 0, 6, 3, 4, 2, 4 }
                    )
                )
            ...
            test("Works with vertical lines.", function()
                local tab = _.polygon.getPolygonIntersection(
                    { 2, 3, 2, 6, 4, 6, 4, 4, 5, 5, 5, 3 },
                    { 3, 2, 3, 5, 6, 4, 6, 3, 4, 3, 4, 2 }
                )
                assert_tables_fuzzy_equal(
                    tab,
                    { { 4, 4.66666 }, { 4.5, 4.5 }, { 5, 4.33333 }, { 5, 3 }, { 3, 3 }, { 4, 3 } }
                )
            end)
        end)
        ...
        context("getCircleIntersection", function()
            test("Returns true if the circle intersects", function()
                ...
                assert_tables_fuzzy_equal(
                    tab,
                    { { "tangent", 5, 4 }, { "tangent", 6, 5 }, { "tangent", 5, 6 }, { "tangent", 4, 5 } }
                )
        ...
        context("isCircleCompletelyInside", function()
            test("Returns if a circle is completely inside of a polygon.", function()
                assert_true(_.polygon.isCircleCompletelyInside(0, 0, 0.2, -0.5, 0, 0, 0.5, 0.5, 0, 0, -0.5))
                assert_false(_.polygon.isCircleCompletelyInside(0.2, 0.2, 0.2, -0.5, 0, 0, 0.5, 0.5, 0, 0, -0.5))
                assert_false(_.polygon.isCircleCompletelyInside(0, 0, 0.4, -0.5, 0, 0, 0.5, 0.5, 0, 0, -0.5))
                assert_false(_.polygon.isCircleCompletelyInside(0, 0, 0.5, -0.5, 0, 0, 0.5, 0.5, 0, 0, -0.5))
                assert_false(_.polygon.isCircleCompletelyInside(0, 0, 1, -0.5, 0, 0, 0.5, 0.5, 0, 0, -0.5))
            end)
        end)
        ...
        context("isPolygonCompletelyInside", function()
            test("Returns if a polygon is completely inside of a polygon.", function()
                assert_true(
                    _.polygon.isPolygonCompletelyInside(
                        { -0.5, 0, 0, 0.5, 0.5, 0, 0, -0.5 },
                        { -0.5, -0.5, -1, -0.5, -1, 0.5, -0.5, 0.5, -0.5, 1, 0.5, 1, 0.5, 0.5, 1, 0.5, 1, -0.5, 0.5, -0.5, 0.5, -1 }
                    )
                )
                assert_false(
                    _.polygon.isPolygonCompletelyInside(
                        { -0.5, 0, 0, 1, 0.5, 0, 0, -0.5 },
                        { -0.5, -0.5, -1, -0.5, -1, 0.5, -0.5, 0.5, -0.5, 1, 0.5, 1, 0.5, 0.5, 1, 0.5, 1, -0.5, 0.5, -0.5, 0.5, -1 }
                    )
                )
                assert_false(
        ...
        context("isSegmentCompletelyInside", function()
            test("Returns if a segment is completely inside of a polygon.", function()
                assert_true(
                    _.polygon.isSegmentCompletelyInside(
                        0,
                        0.5,
                        0.5,
                        0,
    ...
    context("circle", function()
        ...
        context("getLineIntersection", function()
            test("Returns 'Secant' when intersects twice.", function()
                assert_multiple_fuzzy_equal(
                    { _.circle.getLineIntersection(4, 9, 1, 0, 9, 6, 9) },
                    { "secant", 3, 9, 5, 9 }
                )
                assert_multiple_fuzzy_equal(
                    { _.circle.getLineIntersection(2, 2, 1, 2, 3, 3, 2) },
                    { "secant", 2, 3, 3, 2 }
                )
            end)
        ...
        context("getSegmentIntersection", function()
            test("Returns 'Secant' if the line connects two points.", function()
                assert_multiple_fuzzy_equal(
                    { _.circle.getSegmentIntersection(4, 9, 1, 0, 9, 6, 9) },
                    { "secant", 3, 9, 5, 9 }
                )
            end)
            ...
            test("Returns 'Tangent' if the line attaches only one point.", function()
                assert_multiple_fuzzy_equal(
                    { _.circle.getSegmentIntersection(1, 1, 1, 0, 0, 0, 2) },
                    { "tangent", 0, 1 }
                )
            end)
            ...
            test("Returns 'Chord' if both points are on the circle.", function()
                assert_multiple_fuzzy_equal(
                    { _.circle.getSegmentIntersection(0, 0, 1, -1, 0, 1, 0) },
                    { "chord", -1, 0, 1, 0 }
                )
            end)
            ...
            test("Returns 'Enclosed' if the line is within the circle entirely.", function()
                assert_multiple_fuzzy_equal(
                    { _.circle.getSegmentIntersection(0, 0, 2, -1, 0, 1, 0) },
                    { "enclosed", -1, 0, 1, 0 }
                )
            end)
        ...
        context("isCircleCompletelyInside", function()
            test("Returns if a circle is completely inside of another circle.", function()
                ...
                assert_false(_.circle.isCircleCompletelyInside(8, 2, 0.1, 2, 1, 4))
            end)
        end)
        ...
        context("isCircleCompletelyInsidePolygon", function()
            test("Returns if a circle is completely inside of a polygon.", function()
                assert_true(_.circle.isCircleCompletelyInsidePolygon(0, 0, 0.2, -0.5, 0, 0, 0.5, 0.5, 0, 0, -0.5))
                assert_false(_.circle.isCircleCompletelyInsidePolygon(0.2, 0.2, 0.2, -0.5, 0, 0, 0.5, 0.5, 0, 0, -0.5))
                assert_false(_.circle.isCircleCompletelyInsidePolygon(0, 0, 0.4, -0.5, 0, 0, 0.5, 0.5, 0, 0, -0.5))
                assert_false(_.circle.isCircleCompletelyInsidePolygon(0, 0, 0.5, -0.5, 0, 0, 0.5, 0.5, 0, 0, -0.5))
                assert_false(_.circle.isCircleCompletelyInsidePolygon(0, 0, 1, -0.5, 0, 0, 0.5, 0.5, 0, 0, -0.5))
            end)
        end)
    end)
    ...
    context("statistics", function()
        ...
        context("getDispersion", function()
            test("Givse the dispersion.", function()
                assert_multiple_fuzzy_equal(
                    { _.statistics.getDispersion(600, 470, 170, 430, 300) },
                    { 0, 430, 147.32277 }
                )
            end)
        end)
        ...
        context("getVariationRatio", function()
            test("Returns the variation ratio.", function()
                assert_fuzzy_equal(_.statistics.getVariationRatio(4, 2, 5, 3, 0, 4), 0.66667)
            end)
        end)
    end)
    ...
    context("math", function()
        context("getRoot", function()
            test("Gives the nth root to x, given n and x.", function()
                ...
                assert_multiple_fuzzy_equal({ _.math.getRoot(4, -2) }, { 0.5 })
            end)
        end)
        ...
        context("getSummation", function()
            test("Adds up numbers and such.", function()
                assert_equal(
                    _.math.getSummation(1, 10, function(i)
                        return (i * 2)
                    end),
                    110
                )
            ...
            test("Can access previous values with second function argument.", function()
                assert_equal(
                    _.math.getSummation(1, 5, function(i, t)
                        if t[i - 1] then
                            return i + t[i - 1]
                        else
                            return 1
        ...
        context("getPercentage", function()
            test("Gives the percent.", function()
                ...
                assert_equal(_.math.getPercentage(0.5, 50), 25)
                assert_equal(_.math.getPercentage(50, 2), 100)
                assert_equal(_.math.getPercentage(-0.5, 4), -2)
            end)
        end)
        ...
        context("getQuadraticRoots", function()
            ...
            test("Returns false it has no roots.", function()
                ...
                assert_false(_.math.getQuadraticRoots(0.6, 0.3, 0.9))
            end)
        end)
