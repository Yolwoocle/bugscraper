...
local function is_linux()
    ...
    local os_name = os.getenv("OS") or io.popen("uname"):read("*l")
    if (os_name and os_name:match("Linux")) or os.getenv("XDG_CURRENT_DESKTOP") then
        return true
    else
        return false
    end
...
local function creq(path)
    ...
    print(string.format('creq("%s") failed in "%s": %s', path, req_abs_path, ret))
