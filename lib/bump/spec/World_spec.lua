    ...
    describe(":project", function()
        ...
        describe("when the world is not empty", function()
            ...
            describe("when next futureX & Y are passed", function()
                ...
                it(
                    "detects the case where an object was touching another without intersecting, and then penetrates",
                    function()
    ...
    describe(":move", function()
        ...
        describe("when touching", function()
            it("returns a collision with the first item it touches", function()
                ...
                world:add("b", 0, 2, 1, 1)
                world:add("c", 0, 3, 1, 1)
                local x, y, cols, len = world:move(a, 0, 5, function()
                    return "touch"
                end)
                assert.same({ x, y }, { 0, 1 })
                assert.equals(1, len)
        ...
        describe("when sliding", function()
            it("slides with every element", function()
                ...
                world:add("b", 0, 2, 1, 2)
                world:add("c", 2, 1, 1, 1)
                local x, y, cols, len = world:move(a, 5, 5, function()
                    return "slide"
                end)
                assert.same({ x, y }, { 1, 5 })
                assert.equals(1, len)
