...
function EnumOptionMenuItem:init(i, x, y, text, property_name, values, text_format, additional_update)
    ...
    elseif type(text_format) == "string" then
        text_formatter = function(value)
            return Text:text(text_format .. "." .. tostring(value))
        end
...
function EnumOptionMenuItem:find_option_index()
    ...
    print(
        "WARNING: EnumOptionMenuItem: cannot find index of value '"
            .. tostring(self.value or "[nil]")
            .. "' for '"
            .. tostring(self.label_text or "[nil]")
            .. "'"
    )
    return 1
end
