...
function ProgressBarMenuItem:init(i, x, y, params)
    ...
    self.ox = 0
    self.oy = 0
    self.w = CANVAS_WIDTH - MENU_PADDING * 2
...
function ProgressBarMenuItem:draw()
    ...
    if self.overlay_value then
        over_val = (clamp(self.overlay_value, self.min_value, self.max_value) - self.min_value)
    ...
    if self.overlay_value then
        draw_3_slice(
            images.selection_left_small,
            images.selection_right_small,
            COL_LIGHTEST_GRAY,
            x,
            y,
            w * over_val,
