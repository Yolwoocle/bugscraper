...
function GameUI:draw()
    ...
    if self.dark_overlay_alpha > 0 then
        rect_color(
            transparent_color(COL_BLACK_BLUE, self.dark_overlay_alpha),
            "fill",
            0,
            0,
            CANVAS_WIDTH,
...
function GameUI:draw_logo()
    for i = 1, #LOGO_COLS + 1 do
        local ox, oy = cos(game.logo_a + i * 0.4) * 8, sin(game.logo_a + i * 0.4) * 8
...
function GameUI:draw_offscreen_indicators()
    ...
    for i, player in pairs(self.game.all_players) do
        if
            (player.x + player.w < cam_x)
            or (cam_x + CANVAS_WIDTH < player.x)
            or (player.y + player.h < cam_y)
            or (cam_y + CANVAS_HEIGHT < player.y)
        then
...
function GameUI:draw_offscreen_indicator_for(player)
    ...
    print_centered_outline(
        player.skin.color_palette[1],
        player.skin.color_palette[4],
        Text:text("player.abbreviation", player.n),
        x,
        y - 16
    )
end
...
function GameUI:update_cinematic_bars(dt)
    ...
    self.cinematic_bar_scroll = (self.cinematic_bar_scroll + self.cinematic_bar_scroll_speed * dt)
        % self.cinematic_bar_loop_threshold
end
