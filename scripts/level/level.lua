...
function Level:check_for_next_wave(dt)
    local conditions_for_new_wave = (game.game_state == GAME_STATE_PLAYING)
        and (#self.enemy_buffer == 0)
        and (game:get_enemy_count() <= 0)
...
function Level:on_player_leave(player_n) end
...
function Level:update_hole_stencil(dt)
    self.hole_stencil_radius_progress =
        clamp(self.hole_stencil_radius_progress + dt * self.hole_stencil_radius_progress_speed, 0, 1)

    self.hole_stencil_radius = easeinoutquart(
        self.hole_stencil_radius_source,
        self.hole_stencil_radius_target,
        self.hole_stencil_radius_progress
    )
        ...
        if self.hole_stencil_mode ~= "off" then
            ...
            if self.hole_stencil_mode == "hole" then
                love.graphics.circle(
                    "fill",
                    (self.door_rect.ax + self.door_rect.bx) / 2,
                    (self.door_rect.ay + self.door_rect.by) / 2,
                    self.hole_stencil_radius
                )
            end
...
function Level:draw_win_screen()
    ...
    for i = 1, #LOGO_COLS + 1 do
        ...
        for i_chr = 1, #text do
            ...
            local ox, oy = cos(t * 4 + i * 0.2) * 8, sin(t * 4 + i * 0.2) * 8
    ...
    for i = 2, #self.world_generator.end_rubble_slices do
        ...
        for ix = slice.ax, slice.bx do
            if
                ...
                and ((not slice_up) or (slice_up and not slice_up:is_point_in_inclusive(ix, slice.ay - 1)))
            then
