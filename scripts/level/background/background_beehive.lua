...
function BackgroundBeehive:init(level)
    ...
    self.bg_particle_palettes = {
        ...
        {
            COL_BLACK_BLUE,
            { COL_LIGHT_RED, COL_ORANGE, COL_LIGHT_YELLOW, color(0x63c74d), COL_LIGHT_BLUE, color(0xb55088) },
        }, --rainbow
...
function BackgroundBeehive:init_bg_particles()
    self.bg_particles = {}
...
function BackgroundBeehive:set_speed(val)
    self.speed = val
end

function BackgroundBeehive:change_clear_color(force_palette)
...
function BackgroundBeehive:set_clear_color(palette)
...
function BackgroundBeehive:new_bg_particle()
    local o = {}
...
function BackgroundBeehive:update_bg_particles(dt)
    -- Background lines
    for i = 1, #self.bg_particles do
        local o = self.bg_particles[i]
        o.y = o.y + dt * self.speed * o.spd

        local del_cond = (self.speed >= 0 and o.y > CANVAS_HEIGHT + o.h)
            or (self.speed < 0 and o.y - o.h < -CANVAS_HEIGHT)
        if del_cond then
            local p = self:new_bg_particle()
...
function BackgroundBeehive:shift_to_red(speed_cap)
...
function BackgroundBeehive:on_exploding_elevator()
...
function BackgroundBeehive:draw_particle(o)
    if o.type == "rect" then
        local y = o.y + o.oy
