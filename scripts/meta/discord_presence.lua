...
function DiscordPresence:update(dt)
    ...
    self.presence.state =
        ternary(number_players <= 1, Text:text("discord.state.solo"), Text:text("discord.state.local_multiplayer"))
...
function DiscordPresence:ready(user_id, username, discriminator, avatar)
    ...
    print(
        string.format(
            "Discord RPC: ready (user_id %s, username %s, discriminator %s, avatar %s)",
            user_id,
            username,
            discriminator,
            avatar
