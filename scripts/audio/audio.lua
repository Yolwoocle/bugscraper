...
function AudioManager:play(snd, volume, pitch)
    ...
    if not Options:get("sound_on") then
        return
    end
...
function AudioManager:play_pitch(snd, pitch, object)
    ...
    sfx:setPitch(pitch)
    if object then
        self:set_source_position_relative_to_object(sfx, object)
    end
    self:play(sfx)
    --snd:setPitch(1)
end
...
function AudioManager:set_source_position_relative_to_object(source, obj)
    if not source then
        print("AudioManager:set_source_position_relative_to_object : No source defined!")
        return
    end
    if not obj then
        print("AudioManager:set_source_position_relative_to_object : No object defined!")
        return
    end
    local mult = 0.05
    source:setPosition(
        mult * (obj.x - CANVAS_WIDTH * 0.5) / CANVAS_WIDTH,
        mult * (obj.y - CANVAS_HEIGHT * 0.5) / CANVAS_HEIGHT
    )
end
