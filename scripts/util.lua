...
function bounce_vector_cardinal(incoming_x, incoming_y, normal_x, normal_y)
    ...
    if normal_x ~= 0 then
        return sign(normal_x) * abs(incoming_x), incoming_y
    end
    if normal_y ~= 0 then
        return incoming_x, sign(normal_y) * abs(incoming_y)
    end
end
...
function draw_3_slice(img_left, img_right, col, x, y, w, h)
    ...
    exec_color(col, function()
        ...
        rect_color(
            col,
            "fill",
            math.floor(x + img_left:getWidth()),
            math.floor(y),
            math.ceil(w - img_left:getWidth() - img_right:getWidth()),
            h
        )
    end)
end
...
function print_wavy_centered_outline_text(
    col_in,
    col_out,
    text,
    x,
    ...
    for i = 1, #text do
        ...
        print_outline(
            ...
            c,
            ix,
            y + math.sin(wave_t * wave_freq + i * letter_offset) * wave_ampl,
            thick,
            rot,
            sx,
            sy,
...
function line_dotted(col, ax, ay, bx, by, params)
    ...
    while ir <= len do
        if ir + segment_length >= 0 then
            ...
            line_color(col, ax + dx * ir1, ay + dy * ir1, ax + dx * ir2, ay + dy * ir2)
        end
...
function segment_intersect_point(seg1, seg2)
    ...
    return ternary((s >= 0 and s <= 1 and t >= 0 and t <= 1), { x = x1 + t * a_dx, y = y1 + t * a_dy }, nil)
end
...
function segment_intersect(seg1, seg2)
    ...
    local bool1 = (
        ccw(seg1.ax, seg1.ay, seg2.ax, seg2.ay, seg2.bx, seg2.by)
        ~= ccw(seg1.bx, seg1.by, seg2.ax, seg2.ay, seg2.bx, seg2.by)
    )
    local bool2 = (
        ccw(seg1.ax, seg1.ay, seg1.bx, seg1.by, seg2.ax, seg2.ay)
        ~= ccw(seg1.ax, seg1.ay, seg1.bx, seg1.by, seg2.bx, seg2.by)
    )
