        ...
        dying = {
            ...
            update = function(state, dt)
                ...
                if abs(self.spr.rot - goal_r) < 0.1 then
                    ...
                    Particles:splash(
                        self.mid_x,
                        state.death_y - state.death_oy + self.h / 2,
                        40,
                        { COL_LIGHT_YELLOW, COL_ORANGE, COL_LIGHT_RED, COL_WHITE }
                    )
...
function Player:do_invincibility(dt)
    ...
    if self.invincible_time > 0 and game.frames_to_skip <= 0 then
        ...
        self.iframe_blink_freq = (self.invincible_time > 1) and self.iframe_blink_default_freq
            or self.iframe_blink_default_freq / 2
...
function Player:do_damage(n, source)
    ...
    local a = (source ~= nil)
            and (get_angle_between_vectors(
                self.mid_x,
                self.mid_y,
                (source.mid_x or source.x),
                (source.mid_y or source.y)
            ))
        or (random_range(0, pi2))
...
function Player:apply_effect_on_damage()
    ...
    if self.spawn_explosion_on_damage then
        ...
        Particles:image(
            ...
            self.mid_y,
            30,
            { images.pomegranate_piece_1, images.pomegranate_piece_2, images.pomegranate_piece_3 },
            4,
            nil,
            nil,
            nil,
...
function Player:move(dt)
    ...
    local dir = { x = 0, y = 0 }
    if self:action_down("left") then
        dir.x = dir.x - 1
    end
    if self:action_down("right") then
        dir.x = dir.x + 1
    end
    if self.debug_god_mode or self.can_move_360 then
        if self:action_down("up") then
            dir.y = dir.y - 1
        end
        if self:action_down("down") then
            dir.y = dir.y + 1
        end
    end
...
function Player:do_wall_sliding(dt)
    ...
    if self.wall_col then
        ...
        local is_wall_sliding = is_walled
            and is_falling
...
function Player:shoot(dt, is_burst)
    ...
    if self.dir_y ~= 0 and not aim_horizontal then
        dx = 0
    end
...
function Player:is_in_poison_cloud()
    local is_touching, col = self:is_touching_collider(function(col)
        return col.other.is_poisonous
    end)
    if col then
...
function Player:leave_game_if_possible(dt)
    local is_touching, exit_sign = self:is_touching_collider(function(col)
        return col.other.is_exit_sign
    end)
...
function Player:increase_combo(x, y)
    if true then
        return
    end -- Removed combo for now, remove this line if ever needed
...
function Player:update_visuals()
    ...
    if self:is_in_poison_cloud() then
        Particles:dust(
            self.mid_x + random_neighbor(7),
            self.mid_y + random_neighbor(7),
            random_sample({ color(0x3e8948), color(0x265c42), color(0x193c3e) })
        )
    end
...
function Player:draw_hud()
    if self.is_removed or self.is_dead then
        return
    end
    ...
    if self.combo >= self.min_combo_visual_trigger then
        ...
        local s = tostring(self.combo)
        if game.level:is_on_cafeteria() then
            s = "[" .. s .. "]"
        end
...
function Player:draw_ammo_bar(ui_x, ui_y)
    ...
    ui:draw_progress_bar(bar_x, y, bar_w, ammo_icon_w, val, maxval, col_fill, COL_BLACK_BLUE, col_shad, text)
end
