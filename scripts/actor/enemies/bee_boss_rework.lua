...
local LarvaProjectileBoss = require "scripts.actor.enemies.larva_projectile_boss"
local HoneypotLiquid         = require "scripts.actor.enemies.honeypot_liquid"

local BeeBossRework = Enemy:inherit()

function BeeBossRework:init(x, y)
    BeeBossRework.super.init(self, x,y, images.bee_boss_alt_1, 32, 32)
    self.name = "bee_boss_rework_CHANGEME"
    ...
    self.state_machine = StateMachine:new({
        random = {
            ...
            update = function(state, dt)
                self.debug_values[1] = 'aaaaaaaaaaaaaaaaaa' .. tostring(self.boss_phase_number)
                self.boss_phase_number = self.boss_phase_number + 1
                ...
                if self.boss_phase_number % 3 == 0 then
                    return "pong"
                end
        ...
        pong_linger = {
            ...
            update = function(state, dt)
                ...
                self.show_spikes_ball = (self.t % 0.2 < 0.1)
        ...
        thwomp = {
            enter = function(state)
                self.previous_state_name = "thwomp"
        ...
        spawn_minions = {
            enter = function(state)
                self.previous_state_name = "spawn_minions"

                self.vx = 0
                self.vy = 0

                self.state_timer:start(3.5)
                self.spawn_timer = Timer:new(0.4)
            ...
            update = function(state, dt)
                ...
                if self.spawn_timer:update(dt) then
                    local amount = random_range_int(1, 1)
                    for i = 1, amount do
                        if #self.minions < self.max_minions then
                            ...
                            local angle = -pi/2 + random_neighbor(pi/4)
                            local larva_projectile = LarvaProjectileBoss:new(self.mid_x, self.mid_y)
                            table.insert(self.minions, larva_projectile)
        ...
        crowd = {
            enter = function(state)
                ...
                state.target_y = game.level.cabin_inner_rect.ay + self.h + 16
                state.speed = 300
            end,
            update = function(state, dt)
                local vx, vy = normalize_vect(state.target_x - self.x, state.target_y - self.y)
                self:set_position(self.x + vx * state.speed * dt, self.y + vy * state.speed * dt)                

                if dist(self.x, self.y, state.target_x, state.target_y) < 4 then
                    return "crowd_spawn"
                end
            end,
            after_collision = function(state, col, other)
...
function BeeBossRework:ready()
    BeeBossRework.super.ready(self)
    self:spawn_spikes()
end

function BeeBossRework:set_state(state_name)
    self.state_machine:set_state(state_name)
end


function BeeBossRework:update(dt)
    BeeBossRework.super.update(self, dt)
...
function BeeBossRework:draw()
    BeeBossRework.super.draw(self)

    self.state_machine:draw()

    if not self.is_stompable or self.show_spikes_ball then
        draw_centered(images.bee_boss_shield, self.mid_x, self.mid_y)    
    end
end

function BeeBossRework:on_stomped(player)
    game:frameskip(10)
    game:screenshake(8) 
...
function BeeBossRework:on_negative_life()
...
function BeeBossRework:on_death()
end

function BeeBossRework:on_damage_player(player, damage)
end

-----------------------------------------------------

function BeeBossRework:set_spikes_pattern_times(t_off, t_telegraph, t_on)
    for _, spike in pairs(self.spikes) do
        spike:set_pattern_times(t_off, t_telegraph, t_on)
    end
end

function BeeBossRework:set_spikes_length(length)
    for _, spike in pairs(self.spikes) do
        spike:set_length(length)
    end
end

function BeeBossRework:set_spike_waves(direction)
...
function BeeBossRework:set_spikes_pattern_big_wave()
...
function BeeBossRework:after_collision(col, other)
    if col.type ~= "cross" then
        self.state_machine:_call("after_collision", col)
    end
end

function BeeBossRework:spawn_spikes()
    self.spikes = {}
