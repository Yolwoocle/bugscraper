...
function Chipper:is_collision_normal_to_direction(normal)
    ...
    return (
        (dir == 0 and normal.x == -1 and normal.y == 0)
        or (dir == 1 and normal.x == 0 and normal.y == -1)
        or (dir == 2 and normal.x == 1 and normal.y == 0)
        or (dir == 3 and normal.x == 0 and normal.y == 1)
    )
end
    ...
    if col.type ~= "cross" then
        ...
        if self.state_machine:in_state("wander") then
            local new_vx, new_vy = bounce_vector_cardinal(
                -math.cos(self.direction * pi / 2),
                -math.sin(self.direction * pi / 2),
                col.normal.x,
                col.normal.y
            )
