                ...
                if not (self.next_sibling == self) then
                    if
                        ...
                        >= dist(
                            self.signal_start.x,
                            self.signal_start.y,
                            self.next_sibling.mid_x,
                            self.next_sibling.mid_y
                        )
                    then
                        self:switch_next()
                    end
...
function SwitchBug:draw()
    ...
    line_color(
        ternary((self.state_machine.current_state_name == "on"), COL_CYAN, { 1, 1, 1, 0.5 }),
        self.mid_x,
        self.mid_y,
        self.next_sibling.mid_x,
        self.next_sibling.mid_y
    ...
    if self.state_machine.current_state_name == "on" then
        ...
        local type, x1, y1, x2, y2 = mlib.circle.getSegmentIntersection(
            self.signal_start.x,
            self.signal_start.y,
            self.signal_rad,
            self.mid_x,
