...
function Actor:init_actor(x, y, w, h, spr, args)
    ...
    if args.add_collision == nil then
        args.add_collision = true
    end
...
function Actor:update_sprite_position() end
...
function Actor:on_grounded_state_change(new_state) end
...
function Actor:apply_force_from(q, source, ox, oy)
    ...
    local knockback_x, knockback_y =
        normalize_vect(source.x - self.mid_x + ox, (source.mid_y or source.y) - self.mid_y + oy)
    self:apply_force(q, -knockback_x, -knockback_y)
end

function Actor:do_knockback(q, force_x, force_y)
    if not self.is_knockbackable then
        return
    end
...
function Actor:on_leaving_ground() end

function Actor:on_removed() end
