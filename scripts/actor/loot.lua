...
function Loot:init_loot(spr, x, y, w, h, val, vx, vy, params)
    ...
    self.player_filter = param(params.player_filter, function(player)
        return true
    end)
...
function Loot:on_collect(player) end
...
function Loot.Gun:on_collect(player)
    ...
    Particles:word(
        self.mid_x,
        self.y,
        utf8.upper(self.gun.display_name or self.gun.name),
        self.gun.color or COL_LIGHT_YELLOW
    )
    self:reset()
