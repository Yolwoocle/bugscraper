...
function Files:read_config_file(path, reference, create_if_missing)
    ...
    if not love.filesystem.getInfo then
        print(
            "Files:read_config_file - /!\\ WARNING: love.filesystem.getInfo doesn't exist. Either running on web or LÖVE version is incorrect. Loading "
                .. tostring(path)
                .. " aborted, so custom options will not be loaded."
        )
    ...
    if not text then
        print("Files:read_config_file - Error reading '" .. tostring(path) .. "' file: " .. size)
    end
    ...
    if (key ~= "$version") or (val ~= reference["$version"]) then
        ...
        print(
            concat(
                "Files:read_config_file - Incorrect file veresion (",
                tostring(val),
                ") for '",
                path,
    ...
    for key, value in pairs(values) do
        if key ~= "$version" then
            ...
            if type(value) == "number" then
                string_val = tostring(value)
            end
            if type(value) == "boolean" then
                string_val = tostring(value)
            end
            if type(value) == "table" then
                string_val = concatsep(value, ",")
            end
