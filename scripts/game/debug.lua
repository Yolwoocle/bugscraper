...
function Debug:init(game)
    ...
    self.actions = {
        ["f2"] = {
            "toggle collision view mode",
            function()
                self.colview_mode = not self.colview_mode
            end,
            do_not_require_ctrl = true,
        },
        ["f3"] = {
            "view more info",
            function()
        ...
        ["v"] = {
            ...
            function()
                ...
                for i = 0, 19 do
                    table.insert(
                        points,
                        { cx + math.cos(pi / 2 + pi2 * i / 20) * 100, cy + math.sin(pi / 2 + pi2 * i / 20) * 100 }
                    )
                end
            ...
            function()
                for _, actor in pairs(game.actors) do
                    if actor.name == "timed_spikes" then
                        actor:set_time_offset(
                            -dist(actor.mid_x, actor.mid_y, CANVAS_CENTER[1], CANVAS_CENTER[2]) * 0.01 + 3
                        )
                    end
                end
            end,
...
function Debug:gamepadpressed(joystick, buttoncode) end

function Debug:gamepadreleased(joystick, buttoncode) end

function Debug:gamepadaxis(joystick, axis, value) end
...
function Debug:draw_info_view()
    ...
    if self.test_sprite_t > 0.3 then
        self.test_sprite_t = 0.0
        self.test_sprite:set_spritesheet_tile(
            mod_plus_1(self.test_sprite.spritesheet_tile + 1, self.test_sprite.spritesheet_tile_count_x)
        )
    end
    ...
    do
        ...
        for joy, user in pairs(Input.joystick_to_user_map) do
            joystick_user_str =
                concat(joystick_user_str, "{", string.sub(joy:getName(), 1, 4), "... ", ":", user.n, "}, ")
        end
        ...
        for k, player in pairs(game.queued_players) do
            queued_players_str =
                concat(queued_players_str, player.player_n, ": ", param(player.is_pressed, "nil"), ", ")
        end
        ...
        local txts = {
            ...
            concat(
                ...
                #self.game.actors,
                " / ",
                self.game.actor_limit,
                " | nb of enemies: ",
                self.game:get_enemy_count()
            ),
            ...
            concat(
                "cur_cutscene ",
                (game.cutscene == nil) and "[nil]"
                    or string.format(
                        "%s: [%d/%d] (%.1f s/%.1f s) '%s' / (total: %.1f s)",
                        game.cutscene.name,
            ...
            concat("score: ", game.score) .. " / " .. concat("xp: ", Metaprogression:get_xp()) .. " / " .. concat(
                "xp_level: ",
                Metaprogression:get_xp_level()
            ),
        ...
        for i = 1, #txts do
            print_label(txts[i], 0, 0 + txt_h * (i - 1))
        end
    end
...
function Debug:test_info_view_3d_renderer() end

local test_ang = 0
local test_rect =
    Rect:new(CANVAS_CENTER[1] - 26 * 3, CANVAS_CENTER[2] - 13 * 3, CANVAS_CENTER[1] - 4 * 3, CANVAS_CENTER[2] + 20 * 3)
    ...
    for _, e in pairs(self.game.actors) do
        print_outline(
            ...
            concat(
                ...
                ", ",
                math.floor(e.y),
                ternary(e.is_player, concat(" [", math.floor(e.x / 16), ", ", math.floor(e.y / 16), "]"), "")
            ),
            e.x,
            e.y - 10
        )
...
function Debug:draw_colview()
    ...
    for i, it in pairs(items) do
        ...
        rect_color({ 0, 1, 0, 0.2 }, "fill", x, y, w, h)
        rect_color({ 0, 1, 0, 0.5 }, "line", x, y, w, h)
    ...
    if level then
        ...
        rect_color(
            COL_CYAN,
            "line",
            level.cabin_inner_rect.x,
            level.cabin_inner_rect.y,
            level.cabin_inner_rect.w,
            ...
            for iy = 0, layer_canvas:getHeight(), 8 do
                if (math.floor(ix / 8) + math.floor(iy / 8)) % 2 == 0 then
                    rect_color(
                        ...
                        x + ix,
                        y + iy,
                        math.min(8, layer_canvas:getWidth() - ix),
                        math.min(8, layer_canvas:getHeight() - iy)
                    )
                end
            end
        end
