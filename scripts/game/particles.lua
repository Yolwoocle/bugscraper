...
function Particle:on_removed() end
...
function FallingGridParticle:update(dt)
    ...
    print_debug(
        self.orig_y,
        "-",
        math.sin(self.rot_3d),
        "*",
        self.img_side:getHeight(),
        "sin",
        math.sin(self.rot_3d),
        "cos",
        math.cos(self.rot_3d)
    )
end
...
function ParticleSystem:image(x, y, number, spr, spw_rad, life, vs, g, params)
    ...
    for i = 1, number do
        ...
        self:add_particle(
            ImageParticle:new(sprite, x + dx, y + dy, scale, rot, vx, vy, vs, vr, life, g, is_solid, particle_params)
        )
    end
end
...
function ParticleSystem:ejected_player(spr, x, y, vx, vy)
    self:push_layer(PARTICLE_LAYER_FRONT)
    self:add_particle(
        EjectedPlayerParticle:new(
            spr,
            x,
            y,
            vx or (random_sample({ -1, 1 }) * random_range(100, 300)),
            vy or -random_range(400, 600)
        )
    )
    self:pop_layer()
end
