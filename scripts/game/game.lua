...
function Game:new_game(params)
    ...
    self.game_ui.dark_overlay_alpha_target =
        param(params.dark_overlay_alpha_target, self.game_ui.dark_overlay_alpha_target)
    self.menu_blur = 1
...
function Game:update(dt)
    ...
    for i = 1, #self.layers - 1 do
        self.layers[i].blur = Options:get("menu_blur")
            and (self.menu_manager.cur_menu ~= nil)
            and self.menu_manager.cur_menu.blur_enabled
    end
...
function Game:update_debug(dt) end
...
function Game:keypressed(key, scancode, isrepeat)
    if
        ...
        and (love.keyboard.isDown("lctrl") or love.keyboard.isDown("rshift"))
    then
...
function Game:gamepadpressed(joystick, buttoncode)
    ...
    if self.menu_manager then
        self.menu_manager:gamepadpressed(joystick, buttoncode)
    end
    if self.debug then
        self.debug:gamepadpressed(joystick, buttoncode)
    end
end
...
function Game:gamepadreleased(joystick, buttoncode)
    ...
    if self.menu_manager then
        self.menu_manager:gamepadreleased(joystick, buttoncode)
    end
    if self.debug then
        self.debug:gamepadreleased(joystick, buttoncode)
    end
end
...
function Game:gamepadaxis(joystick, axis, value)
    ...
    if self.menu_manager then
        self.menu_manager:gamepadaxis(joystick, axis, value)
    end
    if self.debug then
        self.debug:gamepadaxis(joystick, axis, value)
    end
end
...
function Game:mousepressed(x, y, button, istouch, presses)
    ...
    if self.menu_manager then
        self.menu_manager:mousepressed(x, y, button, istouch, presses)
    end
end
...
function Game:mousereleased(x, y, button, istouch, presses)
    ...
    if self.menu_manager then
        self.menu_manager:mousereleased(x, y, button, istouch, presses)
    end
end

function Game:textinput(text)
    if self.menu_manager then
        self.menu_manager:textinput(text)
    end
end
