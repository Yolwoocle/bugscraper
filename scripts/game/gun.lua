...
function Gun:update(dt)
    if not self.recoil_force then
        self.recoil_force = self.default_jetpack_force * 0.25
    end
...
function Gun:shoot(dt, player, x, y, dx, dy, is_burst)
    ...
    if is_first_fire and self.cooldown_timer > 0 then
        return false
    end
    if is_first_fire and self.reload_timer > 0 then
        return false
    end
...
function Gun:on_shoot(player) end
...
function Gun:do_natural_recharge(dt)
    ...
    if self.natural_recharge_delay_timer > 0.0 then
        return
    end
    ...
    self.natural_recharge_progress =
        math.min(1.0, self.natural_recharge_progress + mult * dt / self.natural_recharge_time)
    local natural_recharge_amount =
        math.floor(self.original_natural_recharge_ammo + self.natural_recharge_progress * self:get_max_ammo())
